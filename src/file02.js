...
for (const string of frameworks){
  const proce = string.split("|").map(proce => proce.trim());
  const [nombre,fecha,count,popularidad] = proce;
  const frameworkObj = {
    frameworkName: nombre,
    releaseDate: fecha,
    usersName: count,
    popularityPercentage: popularidad
